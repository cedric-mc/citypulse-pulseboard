...
// Le backend synchronise à la volée au premier appel
// Render peut prendre jusqu'à 8s pour synchroniser une nouvelle ville
...
const Index = () => {
  ...
  // avant de déclencher l'appel au score — évite le 404
  const scoreReady = !!weather.data || !!air.data;
  const score = useFetch(
    () => getUrbanScore(selectedCity),
    [selectedCity, retryCount, scoreReady]
  );

  // ── Retry automatique ──────────────────────────────────────
  // Si une erreur est détectée et qu'on n'est pas en chargement,
  // on attend RETRY_DELAY ms puis on incrémente retryCount
  // Ce qui force un nouveau fetch sur toutes les données
  // Utile pour les villes inconnues qui nécessitent une sync back
  useEffect(() => {
  ...
  // ── Rafraîchissement manuel + auto toutes les 5 minutes ────
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-6">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          {/* Score Urbain — affiché seulement quand météo ou air disponible */}
          <div className="md:col-span-2">
            {score.loading
              ? <CardLoader />
              : score.error
              ? <CardError message={score.error} onRetry={score.refetch} />
              : score.data && <UrbanScore data={score.data} />}
          </div>
          ...
          <div className="md:col-span-2">
            {weather.loading
              ? <CardLoader />
              : weather.error
              ? <CardError message={weather.error} onRetry={weather.refetch} />
              : weather.data && <WeatherCard data={weather.data} forecast={forecast.data} city={selectedCity} />}
          </div>
          ...
          {air.loading
            ? <CardLoader />
            : air.error
            ? <CardError message={air.error} onRetry={air.refetch} />
            : air.data && <AirQualityCard data={air.data} />}

          {/* Prédictions IA */}
          <PredictionCard city={selectedCity} />

          {/* Carte interactive — reçoit les events pour afficher
              les marqueurs des événements sur la carte */}
          <div className="md:col-span-2">
            <MapSection
              city={selectedCity}
              aqi={air.data?.aqi || 1}
          ...
          <div className="md:col-span-2">
            {events.loading
              ? <CardLoader />
              : events.error
              ? <CardError message={events.error} onRetry={events.refetch} />
              : events.data && <EventsList data={events.data} />}
          </div>
